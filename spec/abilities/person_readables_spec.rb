    ...
    context action do
      ...
      let(:all_accessibles) do
        ...
        when :layer_search then
          if with_deleted_roles
            people.where(groups: { layer_group_id: group.layer_group_id }).uniq
          else
            people.in_layer(group.layer_group)
          end
        when :deep_search then
          if with_deleted_roles
            people.where('groups.lft >= :lft AND groups.rgt <= :rgt',
                         lft: group.lft, rgt: group.rgt).uniq
          else
            people.in_or_below(group.layer_group)
          end
